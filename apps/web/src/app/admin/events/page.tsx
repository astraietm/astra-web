...
function CategoryBadge({ category }: { category: string }) {
  const colors: Record<string, { bg: string; text: string; border: string }> = {
    ...
    COMPETITION: { bg: "bg-amber-400/10", text: "text-amber-300", border: "border-amber-400/20" },
  };
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 pb-2 border-b border-neutral-800/80">
        ...
        <div className="flex items-center gap-3">
          <Link
            ...
            className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-white text-neutral-950 hover:bg-neutral-100 text-xs sm:text-sm font-semibold tracking-wide transition-all shadow-sm active:scale-95"
          >
        ...
        <div className="grid gap-4">
          {events.map((event: any) => {
            ...
            return (
              <div
                key={event.id}
                className="rounded-3xl border border-neutral-800/80 bg-neutral-900/50 backdrop-blur-xl p-5 sm:p-6 shadow-sm hover:border-neutral-700 transition-all"
              >
                <div className="flex flex-col sm:flex-row items-start gap-5">
                  {/* Date block */}
                  <div className="flex-shrink-0 flex flex-col items-center justify-center w-14 h-14 rounded-2xl bg-neutral-800 border border-neutral-700/60 text-white shadow-sm">
                    ...
                    <div className="flex flex-wrap items-center gap-2 mb-2">
                      ...
                      <div className="flex items-center gap-2 ml-auto">
                        <Link
                          ...
                          target="_blank"
                          className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-neutral-800 hover:bg-neutral-700 text-neutral-300 hover:text-white rounded-xl text-xs font-medium border border-neutral-700 transition-colors"
                        ...
                        <Link
                          ...
                          className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-white text-neutral-950 hover:bg-neutral-200 rounded-xl text-xs font-semibold transition-colors shadow-sm"
                        >
          ...
          {events.length === 0 && (
            <div className="rounded-3xl border border-neutral-800 p-16 text-center">
              <p className="text-xs text-neutral-500">No events configured yet.</p>
            </div>
          )}
        </div>
      )}
