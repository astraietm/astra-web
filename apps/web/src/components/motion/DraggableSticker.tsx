...
export const DraggableSticker: React.FC<DraggableStickerProps> = ({
  ...
  if (prefersReduced) {
    return (
      <span
        className={`inline-block font-pixel font-bold uppercase tracking-wider border-2 border-black select-none ${colorMap[color]} ${sizeMap[size]} ${className}`}
  ...
  return (
    <motion.span
      ...
      className={`inline-block font-pixel font-bold uppercase tracking-wider border-2 border-black select-none will-change-transform cursor-grab active:cursor-grabbing ${colorMap[color]} ${sizeMap[size]} ${className}`}
    >
      {children}
    </motion.span>
  );
