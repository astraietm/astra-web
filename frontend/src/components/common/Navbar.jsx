...
const Navbar = () => {
    ...
    return (
      ...
      ======================== */}
            <nav className="hidden md:flex fixed top-4 inset-x-0 z-[100] justify-center px-4 pointer-events-none">
             ...
             rounded-full pl-2 pr-2 py-1 h-[50px] w-full max-w-5xl
             transition-all duration-300"
                >
                    ...
                    <Link
                        to="/"
                        className="relative z-10 flex items-center justify-center w-10 h-10 rounded-full bg-gradient-to-br from-cyan-400 via-purple-500 to-pink-500 hover:scale-105 transition-transform"
                    >
      ...
      ======================== */}
            <nav className="md:hidden fixed top-4 inset-x-0 z-[100] px-4 pointer-events-none flex justify-center items-start">
                ...
                <div className="w-full max-w-[340px] relative pointer-events-auto">
                    {/* Island Header */}
                    <div className="bg-white/5 backdrop-blur-xl border border-white/10 shadow-lg shadow-black/20 rounded-full p-1.5 pl-3 flex items-center justify-between h-[50px] relative z-50">

                        {/* Logo Orb */}
                        <Link to="/" className="relative w-9 h-9 rounded-full overflow-hidden flex items-center justify-center group" onClick={() => setIsOpen(false)}>
                        ...
                        <button
                            ...
                            className="w-9 h-9 rounded-full bg-white/5 border border-white/10 flex items-center justify-center text-gray-400 hover:text-white hover:bg-white/10 transition-colors active:scale-95"
                        >
                    ...
                    <AnimatePresence>
                        {isOpen && (
                            <motion.div
                                ...
                                className="absolute top-[54px] left-0 right-0 bg-black/80 backdrop-blur-xl border border-white/10 rounded-3xl p-1.5 shadow-2xl overflow-hidden z-40 flex flex-col gap-0.5"
                            >
                                {navLinks.map((link, i) => (
                                    <motion.div
                                        key={link.name}
